...
- [ ] Create `tests/conftest.py` overriding `test_ws_path` to `Path(__file__).parent / 'test_ws'` and setting `_PYTEST_COLCON_WS_SENTINEL` before the session for clean-shell verification.
